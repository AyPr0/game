(function () {let $1=``;let $2;let $3=``;let $4=``
    ...
    $1+=`Name: ${psn[0]}<br><br>Year: ${time[4]} | Month: ${$2} | Day: ${time[1]+1}<br><br>Race: ${races[race[0][0]]} | Variant: ${races[race[0]]}<br><br>Compass: ${direction[pos[2]]}`
    $1+=`<br><br>Money: ${stat[0]}<br><br>\${col('#7B7','SKILLS:')}<br>-Name [Category] [Level] (%XP)<br><br>\${col('#7B7','[using]:')}<br>${$3}<br>\${col('#7B7','[all]:')}<br>${$4}`
    fun('menu',$1)
    end(0)
})()
