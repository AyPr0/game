(function () {let $1=[x,x,x,x]//tmpn=0 or 100 (depth), tmp=x(scene), tmp2='in' or 'out'(direction)
    ...
    switch(tmp){
    case x:// standing
        t=`Progress: ${tmpn}%<br><br>You are in a sea, there is a ${$1[0]} ahead. The ${$1[1]} is behind you.`
        n=`\${btn('swim',"tmp='walk';next()")} | \${btn('turn around',"tmpn=100-tmpn;tmp2='${$1[2]}';pos[2]=(pos[2]+4)%8;next()")}<br><br>`
    ...
    case 'walk':// travelling
        ...
        if(tmpn>99){
            ...
            if(tmp2=='in'){n+=` | \${btn('swim around',"pos[2]=1*elm('dlst').value;next()")} to the \${lst('dlst',"${direction[(pos[2]+3)%8]}:${(pos[2]+7)%8},${direction[(pos[2]+5)%8]}:${(pos[2]+1)%8}")} side of the ${$1[0]}`}}
        else{t=`Progress: ${tmpn}%<br><br>You are swimming in the sea, there is a ${$1[0]} ahead. The ${$1[1]} is behind you.`
            n=`\${btn('keep swimming',"next()")} | \${btn('turn around',"tmpn=100-tmpn;tmp2='${$1[2]}';pos[2]=(pos[2]+4)%8;tmp=x;next()")}<br><br>`
            n+=`\${btn('explore',"tmp='explore';next()")}`};break;
    case 'explore':// exploring the area
        t=`You explore the sea. `
        switch(rng(2)){
            case 0:t+=`Nothing happens.`;break;
            case 1:t+=`You find some some shells.`;break;
            case 2:t+=`A fish bites you.`;break;}
