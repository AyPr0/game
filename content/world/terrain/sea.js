(function () {let $1=[x,x,'out']//tmpn=0 or 100 (depth), tmp=x(scene), tmp2='in' or 'out'(direction)
    ...
    case x:// standing
        ...
        n=`\${btn('swim',"tmp='swim';next()")} | \${btn('turn around',"tmpn=100-tmpn;tmp2='${$1[2]}';pos[2]=(pos[2]+4)%8;next()")}<br><br>`
        n+=`\${btn('explore',"tmp='explore';t='You explore the sea and';next()")}`;break;
    case 'swim':// travelling
        tmpn+=10
        if(tmpn>99){
            let $2=``
            ...
            if(rng(9)==0){$3="explore';tmpn+=10;t='As you are swimming,'"}else{$3="swim'"}
            t=`You are swimming in the sea. `
            ...
            n=`\${btn('keep swimming',"tmp='${$3};next()")} | \${btn('turn around',"tmpn=100-tmpn;tmp2='${$1[2]}';pos[2]=(pos[2]+4)%8;tmp=x;next()")}<br><br>`
            n+=`\${btn('explore',"tmp='explore';t='You explore the sea and';next()")}`};break;
