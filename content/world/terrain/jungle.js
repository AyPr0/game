(function () {let $1=[x,x,'out']//tmpn=0 or 100 (depth), tmp=x(scene), tmp2='in' or 'out'(direction)
    ...
    case x:// standing
        ...
        n+=`\${btn('explore',"ptime([20]);tmp='explore';t='You explore the jungle and';next()")}`;break;
    case 'walk':// travelling
        ...
        if(tmpn>99){
            ...
            t=`You are walking in the jungle. `
    ...
    case 'explore':// exploring the area
        switch(rng(2)){
            ...
            if(hp[0]<1){t+=` You lose consciousness.`;n=`${btn("next","tmp='faint!2!world/terrain/jungle!none';next('other/rest')")}`}else{n=`${btn("next","tmp=x;next()")}`};break;
    }
    end()
})()