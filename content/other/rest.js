(function () {let $1=[tmp.arr(0),tmp.arr(1),tmp.arr(2),tmp.arr(3),1,'']//rest type ! data ! next ! tmp
    if($1[0]=='faint'){
        ...
        n=`\${btn('rest',"ptime([0,1*elm('slst').value]);heal(${$1[4]}0*elm('slst').value,1);tmp=tmp.arr(1,${$1[1]}+(1*elm('slst').value));next()")} for \${lst('slst',"1:1,2:2,3:3,4:4,6:6,8:8,10:10")} hours `
